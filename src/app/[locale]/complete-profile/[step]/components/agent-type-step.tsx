...
const AgentTypeCard = ({
  ...
  return (
    ...
    >
      ...
      <p className={clsx({ 'text-2xl font-bold text-white': isActiveType }, 'text-xl text-primary')}>{t(`label.${type}`)}</p>
    </div>
  );
};
    ...
    <div className="flex flex-col place-content-center items-stretch">
      ...
      <div className="m-auto my-12 flex gap-6">
        {AgenTypesList.map((agentType) => (
          <AgentTypeCard type={agentType} setActiveType={handleActiveAgentTypeChange} activeType={activeType === agentType} key={agentType} />
        ))}
      </div>
